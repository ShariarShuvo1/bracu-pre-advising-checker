...
from seatStatusThread import htmlThread, jsonThread
...
class Ui_BasicWindow(object):
    ...
    def labUpdate(self, item):
        day_name = {11: "Sunday", 12: "Monday", 13: "Tuesday", 14: "Wednesday", 15: "Thursday", 16: "Friday", 17: "Saturday"}
        day1 = None
        time1 = None
        day2 = None
        time2 = None
            ...
            if item[10] is not None:
                ...
                for i in range(11, len(item)):
                    if item[i] is not None and day1 is None:
                        ...
                        day2 = day_name[i]
                        time2 = item[i]
                self.course_list.append(Course(item[1], item[2], item[3], item[4], item[5], item[6], item[7], item[8], item[9], item[10], day1, time1, day2, time2))
            else:
                self.labUpdate(item)

    def updateSeatStatus(self):
        raw_data = self.html.find_all('tr')
        inserted_count = 0
        for tr in raw_data:
            td = tr.find_all('td')
            name = str(td[1])[str(td[1]).find('>') + 1:str(td[1]).find('</'):]
            section = str(td[5])[str(td[5]).find('>') + 1:str(td[5]).find('</'):]
            seat_booked = int(str(td[8])[str(td[8]).find('>') + 1:str(td[8]).find('</'):])
            for course in self.course_list:
    ...
    def download(self):
        s = requests.Session()
        self.BasicWindow.setCursor(QCursor(Qt.CursorShape.BusyCursor))
        self.jsonThreadParser = jsonThread()
        self.jsonThreadParser.setter("https://gitfront.io/r/user-6015890/N6zXuFDpUWFu/data-sharing/raw/usisCurrentData.json", self.basic_ui, s)
        self.jsonThreadParser.start()

    def FoundJson(self):
        self.createCourse()
        self.htmlThreadParser = htmlThread()
        self.htmlThreadParser.setter("https://admissions.bracu.ac.bd/academia/admissionRequirement/getAvailableSeatStatus", self.basic_ui)
    ...
    def add_to_table(self, list_text):
        ...
        if current_course.theory_1_day is not None:
            row, column = self.get_row_col(current_course.theory_1_time_start, current_course.theory_1_time_end, current_course.theory_1_day)
            self.setting_table(row, column, current_course.table_string())
        if current_course.theory_2_day is not False:
            row, column = self.get_row_col(current_course.theory_2_time_start, current_course.theory_2_time_end, current_course.theory_2_day)
            self.setting_table(row, column, current_course.table_string())
        if current_course.lab_1_day is not None:
            row, column = self.get_row_col(current_course.lab_1_time_start, current_course.lab_1_time_end, current_course.lab_1_day)
            self.setting_table(row, column, current_course.table_string().split(' - ')[0] + ' - LAB')
        if current_course.lab_2_day is not None:
            row, column = self.get_row_col(current_course.lab_2_time_start, current_course.lab_2_time_end, current_course.lab_2_day)
            self.setting_table(row, column, current_course.table_string().split(' - ')[0] + ' - LAB')
        self.resize_table()
    ...
    def FoundHtml(self):
        self.BasicWindow.setCursor(QCursor(Qt.CursorShape.ArrowCursor))
    ...
    def setupBody(self):
        ...
        # Restart Button
        self.reloadButton = object_builder(QtWidgets.QPushButton(parent=self.centralwidget), (int((self.unitWidth*6.5)+(self.unitWidth*0.5)), 1, self.unitWidth*6, self.unitHeight*4), "Reload", self.font_size, False, "QPushButton::!hover { border: 1px solid white; background-color: rgba(0, 0, 0, 0);} QPushButton::hover {border : 1px solid green; background-color: rgba(0, 0, 0, 0);};", self.reloadButtonClicked, self.pointingHandMouse, "Reload the data from USIS. This will clear all of your list")

        # Disable Seat Limit
        self.seatLimit = object_builder(QtWidgets.QCheckBox(parent=self.centralwidget), (int((self.unitWidth*13)+(self.unitWidth*0.5)), 1, self.unitWidth*10, self.unitHeight*4), "Disable Seat Limit", self.font_size, False, "border: 1px solid white; background-color: rgba(0, 0, 0, 0);", self.seatLimitClicked, self.pointingHandMouse, "You will be able to select sections which does not have any seat left")
        
        # Allow Same Course
        self.sameCourse = object_builder(QtWidgets.QCheckBox(parent=self.centralwidget), (int((self.unitWidth*23.5)+(self.unitWidth*0.5)), 1, self.unitWidth*11, self.unitHeight*4), "Allow Same Course", self.font_size, False, "border: 1px solid white; background-color: rgba(0, 0, 0, 0);", self.sameCourseClicked, self.pointingHandMouse, "You will be able to select same course multiple times")

        # Allow Exam Clash
        self.examClash = object_builder(QtWidgets.QCheckBox(parent=self.centralwidget), (int((self.unitWidth*35)+(self.unitWidth*0.5)), 1, self.unitWidth*10, self.unitHeight*4), "Allow Exam Clash", self.font_size, False, "border: 1px solid white; background-color: rgba(0, 0, 0, 0);", self.examClashClicked, self.pointingHandMouse, "You will be able to select courses with same exam date")

        # Allow Theory Clash
        self.theoryClash = object_builder(QtWidgets.QCheckBox(parent=self.centralwidget), (int((self.unitWidth*45.5)+(self.unitWidth*0.5)), 1, self.unitWidth*11, self.unitHeight*4), "Allow Theory Clash", self.font_size, False, "border: 1px solid white; background-color: rgba(0, 0, 0, 0);", self.theoryClashClicked, self.pointingHandMouse, "You will be able to select courses with same theory schedule")

        # Allow Lab Clash
        self.labClash = object_builder(QtWidgets.QCheckBox(parent=self.centralwidget), (int((self.unitWidth*57)+(self.unitWidth*0.5)), 1, self.unitWidth*11, self.unitHeight*4), "Allow Lab Clash", self.font_size, False, "border: 1px solid white; background-color: rgba(0, 0, 0, 0);", self.labClashClicked, self.pointingHandMouse, "You will be able to select courses with same lab schedule")
        self.labClash.setChecked(True)
        
        # Disable all restriction
        self.disableAllRestriction = object_builder(QtWidgets.QCheckBox(parent=self.centralwidget), (int((self.unitWidth*68.5)+(self.unitWidth*0.5)), 1, self.unitWidth*12, self.unitHeight*4), "Disable All Restriction", self.font_size, False, "border: 1px solid white; background-color: rgba(0, 0, 0, 0);", self.disableAllRestrictionClicked, self.pointingHandMouse, "There will be no restriction to add courses")
    ...
    def setupUi(self, BasicWindow, MainWindow, basic_ui):
        ...
        BasicWindow.resize(self.width, self.height)
        BasicWindow.setMinimumSize(QtCore.QSize(self.width, self.height))
        BasicWindow.setBaseSize(QtCore.QSize(self.width, self.height))
        BasicWindow.setStyleSheet("background-color: black; color: yellow")
        BasicWindow.setWindowTitle("Pre Advising Basic")
        self.centralwidget = QtWidgets.QWidget(parent=BasicWindow)
    ...
    def show_description(self, course_name, section):
        ...
        self.details_listViewer.addItem(f'Mid-Term Time: ({current_course.exam_time_start} - {current_course.exam_time_end})')
        ...
        self.details_listViewer.addItem(f'Theory Class Day 1 Time: ({current_course.theory_1_time_start} - {current_course.theory_1_time_end})')
        if current_course.theory_2_day is not None:
            ...
            self.details_listViewer.addItem(f'Theory Class Day 2 Time: ({current_course.theory_2_time_start} - {current_course.theory_2_time_end})')
        self.details_listViewer.addItem("")
        if current_course.labAvailable:
            if current_course.lab_2_day is not None:
                self.details_listViewer.addItem(f'Lab Day: {current_course.lab_1_day}')
                self.details_listViewer.addItem(f'Lab Time First Half: ({current_course.lab_1_time_start} - {current_course.lab_1_time_end})')
                self.details_listViewer.addItem(f'Lab Time Second Half: ({current_course.lab_2_time_start} - {current_course.lab_2_time_end})')
            else:
                self.details_listViewer.addItem(f'Lab Day 1: {current_course.lab_1_day}')
                self.details_listViewer.addItem(f'Lab Day 1 Time: ({current_course.lab_1_time_start} - {current_course.lab_1_time_end})')
    ...
    def bound_checker(self, item):
        ...
        if not isChecked(self.theoryClash):
            for i in range(len(self.right_listViewer)):
                ...
                if self.founded_item.theory_1_day in temp_list and (self.founded_item.theory_1_time_start in temp_list or self.founded_item.theory_1_time_end in temp_list):
                    showWarning(f"Your Theory Class Clashes with\n{temp_course}")
                    self.bound[3] = False
                    break
                if self.founded_item.theory_2_day is not None:
                    if self.founded_item.theory_2_day in temp_list and (self.founded_item.theory_2_time_start in temp_list or self.founded_item.theory_2_time_end in temp_list):
                        showWarning(f"Your Theory Class Clashes with\n{temp_course}")
                        self.bound[3] = False
                        break
                if temp_course.theory_2_day is not None:
                    temp_list = [temp_course.theory_2_day, temp_course.theory_2_time_start, temp_course.theory_2_time_end]
                    if self.founded_item.theory_1_day in temp_list and (self.founded_item.theory_1_time_start in temp_list or self.founded_item.theory_1_time_end in temp_list):
                        showWarning(f"Your Theory Class Clashes with\n{temp_course}")
                        self.bound[3] = False
                        break
        ...
        if not isChecked(self.labClash):
            for i in range(len(self.right_listViewer)):
                temp_course: Course = self.find_course_by_string(self.right_listViewer.item(i).text())
                temp_list = [temp_course.theory_1_day, temp_course.theory_1_time_start, temp_course.theory_1_time_end]
                if self.founded_item.lab_1_day in temp_list and (self.founded_item.lab_1_time_start in temp_list or self.founded_item.lab_1_time_end in temp_list):
                    showWarning(f"Your LAB Class Clashes with\n{temp_course}")
                    self.bound[3] = False
                    break
                if self.founded_item.lab_2_day is not None:
                    if self.founded_item.lab_2_day in temp_list and (self.founded_item.lab_2_time_start in temp_list or self.founded_item.lab_2_time_end in temp_list):
                        showWarning(f"Your LAB Class Clashes with\n{temp_course}")
                        self.bound[3] = False
                        break
                if temp_course.theory_2_day is not None:
    ...
    def backButtonClicked(self):
        ...
        self.BasicWindow.hide()

    def seatLimitClicked(self):
        if not isChecked(self.seatLimit):
            self.disableAllRestriction.setChecked(False)
        if isChecked(self.labClash) and isChecked(self.theoryClash) and isChecked(self.examClash) and isChecked(self.seatLimit) and isChecked(self.sameCourse):
